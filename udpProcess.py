...
import yaml
from udp.network import RSN
import torchvision
from torchvision.models.detection import FasterRCNN_ResNet50_FPN_Weights

class UDP_pose:
    def __init__(self):
        with open('./udp/config.yaml', 'r', encoding='utf8') as f:
            self.config = yaml.safe_load(f)
        self.device = torch.device(self.config['device'])
        self.model = RSN(self.config).cuda()
        self.model.load_state_dict(
            torch.load(self.config['model_file'], map_location=lambda storage, loc: storage)['model'])
        self.model.eval()
        self.means = torch.tensor(self.config['input_means']).cuda()
        self.stds = torch.tensor(self.config['input_stds']).cuda()
    ...
    def udpPose(self, img_np: np):
        ...
        # todo 17 joints minus 5 Facial joints, slice needs to be verified
        return np.array(joints[5:])