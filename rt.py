...
def compute_relative_rt(image_dir_list: List[Tuple[str, str, str, str]], image_size: Tuple[int, int] = (1280, 720), unit: float = 50, x_num: int = 11, y_num: int = 11, distCoeffs: Dict[str, np.ndarray] = {}, cameraMatrix: Dict[str, np.ndarray] = {}, calibrate_flag: int = cv.CALIB_RATIONAL_MODEL, pnp_flag: int = cv.SOLVEPNP_ITERATIVE, calibrate_iterations: int = 50, calibrate_termination_eps: float = 1e-10):
    ...
    for image_dir in unique_image_dir_list:
        p2d_list = []
        for image_path in tqdm(sorted(os.listdir(image_dir)), desc=f"识别 {os.path.basename(image_dir)} 内棋盘格", leave=False):
        ...
        if mtx is not None and dist is not None:
            ...
            ret, mtx, dist, rvecs, tvecs = cv.calibrateCamera(
                exist_corner_p3d_list, exist_corner_p2d_list, (W, H), None, None, flags=calibrate_flag, criteria=(cv.TERM_CRITERIA_COUNT | cv.TERM_CRITERIA_EPS, calibrate_iterations, calibrate_termination_eps))  # 标定
    ...
    for t in image_dir_list:
        ...
        error0_1_2d_list = []
        error1_0_2d_list = []
        for c03d, c13d, p2d0, p2d1 in tqdm(zip(test_cam_p3d_list_d[test_image0_dir], test_cam_p3d_list_d[test_image1_dir], test_p2d_list_d[test_image0_dir], test_p2d_list_d[test_image1_dir]), desc=f'计算误差 {os.path.basename(image0_dir)} {os.path.basename(image1_dir)}', leave=False):
            ...
            cam13d_cam03d_cam02d, _ = cv.projectPoints(cam13d_cam03d, np.eye(3), np.zeros(3), cam0, dist0)  # 1相机3d->0相机3d->0相机2d 
            cam03d_cam13d_cam12d, _ = cv.projectPoints(cam03d_cam13d, np.eye(3), np.zeros(3), cam1, dist1)  # 0相机3d->1相机3d->1相机2d
